...
> ERP real embutido. Mantenha os dois em nomes/pastas distintos se
...
"chutar" nomenclatura de um ERP proprietário. Para T-SQL genérico e para
...
- mencionar um ERP com tabelas nomeadas por código (prefixo de letra única + nome — ex: `F_FUNCIONARIO`);
- pedir dados de domínios típicos de RH/ERP: funcionários, folha de pagamento, ponto eletrônico, cadastros gerais (empresa/unidade), usuários do sistema, recrutamento;
...
```bash
python3 scripts/nomi_nomi_no_mi.py detalhar F_FUNCIONARIO <TABELA2> <TABELA3> ...
```
...
**Nomenclatura de ERPs de código:** muitas FKs usam campos compostos (ex: `CODEMPRESA,CHAPA`) — trate como chave composta no JOIN, uma condição `ON` por campo.

**Regra de negócio, tela ou processo do sistema de origem:** o motor só retorna metadados estruturais (tabela/coluna/FK), nunca regras de negócio — para isso, consulte a documentação oficial do seu próprio ERP.
...
│   └── testes_regressao.py       # suite de testes (gera fixture fictícia própria — ver contextos/README.md)
├── contextos/
