...
"""
================================================================================
 TESTES DE REGRESSÃO — Nomi Nomi no Mi (motor de descoberta de schema)
================================================================================
...
Uso:
    python3 testes_regressao.py                          # usa a fixture fictícia embutida
    python3 testes_regressao.py --db /caminho/schema.db   # usa um banco já importado (seu schema real)
    python3 testes_regressao.py -v                        # mostra o ranking completo de cada caso
...
--------------------------------------------------------------------------------
TRÊS GRUPOS, POR PAPEL METODOLÓGICO — não misturar ao calibrar
--------------------------------------------------------------------------------
...
from nomi_nomi_no_mi import NomiNomiNoMi, DEFAULT_DB_PATH, criar_schema  # noqa: E402
...
def _gerar_filler(n_por_modulo=40, seed=42):
    random.seed(seed)
    ...
    for letra in ('N', 'T', 'F', 'S'):
        for i in range(n_por_modulo):
            ...
            n_cols = random.randint(3, 9)
            termos = random.sample(pool, k=min(n_cols, len(pool)))
            cols = [('#', '')] + [(f"COD{t.upper()}", f"{t.capitalize()} de controle interno") for t in termos]
            tabelas.append((nome, cols))
    return tabelas
...
def gerar_fixture_db(caminho: str, n_filler_por_modulo=40, seed=42):
    ...
    for nome, cols in _gerar_filler(n_filler_por_modulo, seed):
    ...
    for nome, _ in _gerar_filler(n_filler_por_modulo, seed):
        rels.append((nome, 'CODEMPRESA_FK', 'N_EMPRESA', 'CODEMPRESA'))
    ...
    for codsistema, nomesistema, descricao in _SISTEMAS:
        cur.execute(
            "INSERT INTO sistemas (codsistema, nomesistema, descricao) VALUES (?,?,?)",
            (codsistema, nomesistema, descricao)
        )
    for termo, sinonimo in _SINONIMOS:
        cur.execute("INSERT INTO sinonimos (termo, sinonimo) VALUES (?,?)", (termo, sinonimo))

    # tabelas: total_registros plausível + campos derivados (mesma
    # fórmula de `importar_csvs`, replicada aqui para a fixture não
    # depender de reabrir o banco por um segundo processo)
    filhas_por_tabela = defaultdict(set)
    pais_por_tabela = defaultdict(set)
    for tabela_filha, _campo_filho, tabela_mae, _campo_mae in rels:
    ...
    for tabela in todas_tabelas:
        if tabela in _REGISTROS_ESPECIAIS:
            total_reg = _REGISTROS_ESPECIAIS[tabela]
        elif tabela in _TABELAS_NOMEADAS:
            total_reg = random.randint(50, 5000)
        else:
            total_reg = random.randint(0, 2000)
        n_filhas = len(filhas_por_tabela.get(tabela, ()))
        n_pais = len(pais_por_tabela.get(tabela, ()))
        metricas[tabela] = {
            'total_registros': total_reg, 'tabelas_filhas': n_filhas,
            'tabelas_pais': n_pais, 'conexoes_totais': n_filhas + n_pais,
        }
    ...
    for tabela, m in metricas.items():
        cur.execute(
            """INSERT INTO tabelas
               ...
               VALUES (?,NULL,?,?,?,?,?,?)""",
            (tabela, score_por_tabela[tabela], ranking_por_tabela[tabela], m['total_registros'],
...
def rodar(db_path: str, top_n_override: int = None, verbose: bool = False) -> bool:
    ...
    print(f"RESULTADO FINAL: {passou_total}/{total} ({passou_total/total*100:.0f}%)")
    print(f"  Calibração:     {p1}/{t1} ({p1/t1*100:.0f}%)")
    print(f"  Held-out:       {p2}/{t2} ({p2/t2*100:.0f}%)")
    print(f"  Validação cega: {p3}/{t3} ({p3/t3*100:.0f}%)  <- número mais confiável de generalização")
    if todas_falhas:
        print("\nCasos que falharam:")
...
def main():
    parser = argparse.ArgumentParser(description="Testes de regressão do Nomi Nomi no Mi")
    parser.add_argument(
        '--db', default=None,
